//=============================================================================
// ZodiacGrey2: These are the fierce ones in the final mission.  (ZodiacGrey's
// are used in the cloning labs mission)
//=============================================================================
class ZodiacGrey2 extends Animal;
                        ...
                        bool bPlayAnim)
{
	...
	if (actualDamage > 0)
	{
		...
		if (DamageType == 'EnergyWeapon')
	      {
			catchOnFireRandom = FRand();

			// If the Grey is hit with EnergyWeapon damage (JOSHUA rifle), there's
	            // a one in ten chance of catching on fire
			if ((catchOnFireRandom > 0.9) && !bOnFire)
			{
				actualDamage = actualDamage * 0.01;  // reduce damage so we kill the Grey via fire only :)
				CatchFire();
			}
			else
			{
				// If the Grey didn't catch on fire, have JOSHUA rifle ("EnergyWeapon" damage) do 10% more damage
